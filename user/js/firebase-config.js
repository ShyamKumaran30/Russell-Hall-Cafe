...
async function universalLogin(email, password, expectedPortal) {
  ...
  try {
    ...
    if (expectedPortal === 'admin' && typeof initAdminDashboard === 'function') {
      initAdminDashboard();
    } else if (expectedPortal === 'kitchen' && typeof initKitchenPortal === 'function') {
      initKitchenPortal();
    } else if (expectedPortal === 'customer' && typeof initCustomerPortal === 'function') {
      initCustomerPortal();
    }
...
async function seedTablesIfEmpty() {
  // Purged: live tables are removed
  return;
}
